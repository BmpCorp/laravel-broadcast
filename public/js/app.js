...
/* harmony import */ var _task__WEBPACK_IMPORTED_MODULE_1__ = __webpack_require__(/*! ./task */ "./resources/js/task.js");
...
axios.get('/api/task').then(function (response) {
  ...
  if (active) {
    var taskHtml = active.map(_task__WEBPACK_IMPORTED_MODULE_1__["default"].getTemplate);
    $('.active-tasks').append(taskHtml);
  }

  if (completed) {
    var _taskHtml = completed.map(_task__WEBPACK_IMPORTED_MODULE_1__["default"].getTemplate);

    $('.completed-tasks').append(_taskHtml);
  }
}); // Добавить

$('.new-task').on('click', function () {
  var name = prompt('Введите название: ', 'Новая задача');
...
$('body').on('click', '.task__edit', function () {
  var $taskRow = $(this).parents('.task');
  var id = $taskRow.data('id');
  var newName = prompt('Введите новое название: ', $taskRow.find('.task__name').text());

  if (!!newName) {
    axios.post("/api/task/".concat(id, "/update"), {
      name: newName
    }).then(function (response) {
...
$('body').on('click', '.task__delete', function () {
  ...
  if (confirm('Вы действительно хотите удалить задачу?')) {
    axios.post("/api/task/".concat(id, "/delete")).then(function (response) {
      $taskRow.remove();
    });
  }
}); // Обработка событий

echo.channel('tasks').listen('.task-monitor', function (event) {
  switch (event.type) {
    case 'add':
    case 'update':
      _task__WEBPACK_IMPORTED_MODULE_1__["default"].show(event);
      break;

    case 'remove':
      _task__WEBPACK_IMPORTED_MODULE_1__["default"].remove(event.id);
      break;
  }
});
...
var Task = /*#__PURE__*/_createClass(function Task() {
  ...
  _defineProperty(this, "show", function (task) {
    var $existing = jquery__WEBPACK_IMPORTED_MODULE_0___default()(".task[data-id=".concat(task.id, "]"));

    if ($existing.length > 0) {
      $existing.remove();
    }

    jquery__WEBPACK_IMPORTED_MODULE_0___default()(task.is_complete ? '.completed-tasks' : '.active-tasks').append(_this.getTemplate(task));
  });

  _defineProperty(this, "remove", function (id) {
    jquery__WEBPACK_IMPORTED_MODULE_0___default()(".task[data-id=".concat(id, "]")).remove();
  });

  _defineProperty(this, "getTemplate", function (task) {
    return "<tr class=\"task row\" data-id=\"".concat(task.id, "\">\n                    <td class=\"col-1\">\n                        <div class=\"form-check\">\n                            <input class=\"form-check-input task__complete\" type=\"checkbox\"").concat(task.is_complete ? ' checked' : '', ">\n                        </div>\n                    </td>\n                    <td class=\"col-7 task__name\">").concat(task.name, "</td>\n                    <td class=\"col-2\">\n                        <button type=\"button\" class=\"btn btn-primary task__edit\">\u0418\u0437\u043C\u0435\u043D\u0438\u0442\u044C</button>\n                    </td>\n                    <td class=\"col-2\">\n                        <button type=\"button\" class=\"btn btn-danger task__delete\">\u0423\u0434\u0430\u043B\u0438\u0442\u044C</button>\n                    </td>\n                </tr>");
  });
});
